...
此前 `resolveAiEnabled` 在群未出现在 `groupRules` 时**回落到全局 `enableAiReply`**，
导致任何未配置的群也会触发 AI 回复。

现在改为三段判定：

1. 该群有独立配置 → 以其 `enableAiReply` 为准；
2. 未单独配置 → **默认不启用**（新配置项 `aiReplyRequireGroupRule`，默认 `true`）；
3. 仅当把 `aiReplyRequireGroupRule` 显式设为 `false` 时，才回落到全局开关。

### 2. 违禁词白名单（按群配置）

用于校学生会招新这类场景：招新文案本身含「加群」「群号」等违禁词，但属于允许的宣传。

- 新增群级配置 `whitelistKeywords`：消息命中其中**任意一个**关键词即整条跳过违禁词评分；
- 新增 `whitelistOnlyOwners`：设为 `true` 时仅群主/管理员可享用白名单；
- 白名单**只豁免违禁词**，卡片消息与合并转发的拦截不受影响；
- 仅在配置了白名单的群生效，不影响其他群；
- 命中时写日志：`[whitelist] 命中「宣传部」，跳过违禁词管控`。

### 3. 自定义命令

新增 `customCommands` 配置表：检测到指定命令后输出预设消息。

- **触发方式为完全匹配**：触发词需用方括号包裹（如 `[签到]`），
  整条消息与之完全一致（忽略首尾空白）才触发；`[签到] 一下`、`签到` 均不触发；
- **回复内容**支持多条，并支持变量占位符：
  `{user}` 发送者昵称、`{at}` @发送者、`{group}` 群号；
- **每条命令可独立配置**：
  - `randomReply`：多条回复时随机选（关闭则按顺序轮换）；
  - `quote`：回复时引用触发的那条消息；
  - `groups`：限定生效的群，留空为所有群；
  - `cooldownSeconds`：同群冷却时间；
  - `logHit`：命中是否写日志；
- **完全不走 AI**：命中后直接结束本轮处理，违禁词管控与 AI 回复均跳过
  （冷却期内的命中同样不交给 AI）。
...
图片检测与 NapCat/Koishi 实际下发的消息结构完全不匹配，AI 永远「看不到图」：

| 旧实现假设 | 实际结构 |
| --- | --- |
| `session.content` 含 `[CQ:image,...]` | HTML 形态 `<img src="..." summary="[动画表情]" sub-type="1"/>` |
| 元素 `type === 'image'` | `type === 'img'` |
| 地址在 `url` / `src` | 在 `attrs.src` |
| 表情包靠 `[CQ:face]` | `attrs.subType === 1` / `summary="[动画表情]"` |

且 `<img src>` 内 `&amp;` 为 HTML 实体，未解码请求必然失败。

- 新增统一视觉解析器，兼容 CQ / HTML `<img>` / element 三种形态，自动解码 HTML 实体；
- 图片下载并转为 base64 data URL 后再送模型，不依赖模型侧访问 QQ 图链（需 `rkey` 且有时效）；
- 支持「回复某张图片再 @bot」：从 `session.quote`（适配器已取回原消息）中提取图片；
- 表情包（`subType=1`）同样参与识别。

### 2. 随机/阈值触发支持纯图片与表情包

- 纯图片/表情消息此前被「无文本」守卫丢弃，现参与兴趣/阈值判定；
- 兴趣判定提示词不再将「无文字」视为无价值。

### 3. 修复兴趣判定长期失效

带思考模型（如 `deepseek-flash`）先消耗 reasoning tokens，原 `max_tokens: 48` 导致正文为空、判定恒为 `reason=empty`。

- 额度提升至 512（OpenAI 兼容与 Gemini 双路径）；
- 正文为空时回退从 `reasoning_content` 提取 JSON。

### 4. 违禁词改为整条消息评分制

原逐词 `includes` 精确匹配，广告隔词即可绕过（`领红包` 命中不了 `领xxx元红包`）。

- **顺序子序列匹配**：字符按序出现即命中，允许隔词；
- **置信度衰减**：按词长归一化的间隔溢出衰减；跨度 > 词长×3 或溢出 > 8 字直接拒绝（拦 `领 导 强 调 红 色 包 装`）；
- **词长自适应门槛**：短词需更高置信度；
- **可配置权重**：`"关键词|分值"`，未指定走内置分档表；
- **累计得分 ≥ `bannedWordScoreThreshold`（默认 70）** 才触发管控，日志输出命中明细与得分。

> 原理性局限：字符级匹配无法区分字符顺序与间隔完全一致的两个串
> （`领xxx元红包` 与 `领导说把红色包装袋收好` 跨度均为 7、置信度均为 0.667）。
