...
class QQGroupManagerService extends koishi_1.Service {
    ...
    resolveAiEnabled(session) {
        ...
        // 该群有独立配置：以其 enableAiReply 为准
        if (typeof rule?.enableAiReply === 'boolean')
            return rule.enableAiReply;
        // 未单独配置的群：默认不启用 AI 回复（避免全局开关扩散到所有群）
        // 如需恢复旧的「回落到全局」行为，把 aiReplyRequireGroupRule 设为 false
        if (this.config.aiReplyRequireGroupRule !== false)
            return false;
