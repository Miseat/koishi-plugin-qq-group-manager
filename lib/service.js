...
class QQGroupManagerService extends koishi_1.Service {
    ...
    async canUseAdminCommand(session) {
        ...
        // bot 只要具备群主或管理员身份，就有能力执行禁言/踢人等操作。
        // 此前要求必须是 owner，导致绝大多数群（bot 只是管理员）里群管指令全部不可用。
        if (botRole === 'owner' || botRole === 'admin') {
            const allowed = { ok: true, message: `bot(${botId}) 在当前群角色=${botRole}，具备管理能力。` };
            this.logAuth('admin-visibility', allowed, session);
            return allowed;
        }
        const denied = {
            ok: false,
            message: `bot(${botId}) 在当前群角色=${botRole ?? 'unknown'}，既不是群主也不是管理员，无法执行群管操作。`,
            userMessage: '当前群中 bot 既不是群主也不是管理员，无法执行群管操作。',
        };
        this.logAuth('admin-visibility', denied, session);
        return denied;
    }
    ...
    /** 权限不足提示 */
    async denyAdmin(session, message) {
        await session.send(this.styleText(message || '你没有权限执行该操作。'));
        return true;
    }
    /**
     * 群管指令的统一鉴权（两步）：
     *   ① authorizeCommand —— 判定「谁在操作」：账号白名单 / 群主 / 群管理员；
     *   ② canUseAdminCommand —— 确认「bot 在该群是否具备管理能力」。
     *
     * 此前方括号指令只做了第②步，导致 allowedUserIds / allowGroupOwner /
     * allowGroupAdmin 对 mute/unmute/kick/ban/unban 完全失效：配置里允许群主操作，
     * 但只要 bot 不是群主就会被拒；反过来当 bot 是群主时，任何群成员都能踢人。
     */
    async authorizeModerationAction(command, session) {
        const auth = await this.authorizeCommand(command, session);
        if (!auth.ok) {
            this._logger.warn(`[auth:${command}] 拒绝（操作者权限不足）: ${auth.message} | ${this.sessionTag(session)}`);
            return { ok: false, message: auth.userMessage || '你没有权限执行该操作。' };
        }
        const capability = await this.canUseAdminCommand(session);
        if (!capability.ok) {
            this._logger.warn(`[auth:${command}] 拒绝（bot 无管理能力）: ${capability.message} | ${this.sessionTag(session)}`);
            return { ok: false, message: capability.userMessage || '你没有权限执行该操作。' };
        }
        return { ok: true, message: auth.message };
    }
    /** [mute] qq号|@某人 时长 [原因] */
    async execMute(session, rest, mentions = []) {
        ...
        const auth = await this.authorizeModerationAction('mute', session);
        if (!auth.ok)
            return this.denyAdmin(session, auth.message);
    ...
    async execUnmute(session, rest, mentions = []) {
        ...
        const auth = await this.authorizeModerationAction('unmute', session);
        if (!auth.ok)
            return this.denyAdmin(session, auth.message);
    ...
    async execKick(session, rest, mentions = []) {
        ...
        const auth = await this.authorizeModerationAction('kick', session);
        if (!auth.ok)
            return this.denyAdmin(session, auth.message);
    ...
    async execBanAdd(session, rest, mentions = []) {
        ...
        const auth = await this.authorizeModerationAction('ban', session);
        if (!auth.ok)
            return this.denyAdmin(session, auth.message);
        const tokens = rest.split(/\s+/).filter(Boolean);
        let qq = '';
    ...
    async execUnban(session, rest, mentions = []) {
        ...
        const auth = await this.authorizeModerationAction('unban', session);
        if (!auth.ok)
            return this.denyAdmin(session, auth.message);
        const tokens = rest.split(/\s+/).filter(Boolean);
        let qq = '';
    ...
    async execBanList(session) {
        ...
        const auth = await this.authorizeCommand('banlist', session);
        if (!auth.ok) {
            await session.send(this.styleText(auth.userMessage || '你没有权限执行该操作。'));
            return;
        }
        const list = await this.getBlacklist(session);
        if (!list.length) {
